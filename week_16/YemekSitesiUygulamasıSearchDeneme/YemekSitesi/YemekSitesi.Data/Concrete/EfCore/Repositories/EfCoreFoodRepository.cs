...
namespace YemekSitesi.Data.Concrete.EfCore.Repositories
{
    public class EfCoreFoodRepository : EfCoreGenericRepository<Food>, IFoodRepository
    {
        ...
        public async Task<List<Food>> GetSearchResultsAsync(string searchString, bool isApproved = true)
        {
            searchString = searchString.ToLower();
            var result = YemekSitesiContext.Foods.AsQueryable();
            if (isApproved)
            {
                //onaylıysa gözükücek
                return await result
                 .Where(f => f.IsApproved && (f.Name.ToLower().Contains(searchString) ||
                 f.Description.ToLower().Contains(searchString))).ToListAsync();//ismin küçük harfine bak dedik (onaylı olması lazım ve ardığımız ismin geçmesi lazım veya açıklama da içeriyorm)


            }
            //onaylı olsada olmasada gözükücek
            return await result
            .Where(f => f.Name.ToLower().Contains(searchString) ||
            f.Description.ToLower().Contains(searchString)).ToListAsync();
        }
