    ...
    public class CardController : Controller
    {
        ...
        public async Task<IActionResult> Checkout(OrderDto orderDto)
        {
            if (ModelState.IsValid)
            {
                ...
                //ödeme işlemleri ile ilgili kodları buraya yazıcagız
                //1)Kredi kartı numarası kontrolü
                //2)Eger kart numarası geçerliyse ödemeyi al
        ...
        [NonAction] //Action olmamasını saglıyoruz
        private async void SaveOrder(OrderDto orderDto,string userId)
        {
            ...
            order.OrderState = EnumOrderState.Unpaid;//buayı geçici olarak yapıyoruz ödeme taammlanmış bir şekilde gelmemiz lazım
            order.OrderType = EnumOrderType.CrediCard;
        ...
        [NonAction]
        //KART Numarası geçerliyse bu metot çalıştırsın
        private bool CardNumberControl(string cardNumber)
        {
            #region Command

            /*
             * cardNumber boşluk karakterlrinden temzizlensin
             * cardNumberın uzunluk kontrolü
             * Sayısal deger kontrolü
             * Lunh algoritmasını uygulamak için gerekenleri yap
             * */
            #endregion

            cardNumber = cardNumber.Replace("-", "").Replace(" ", "");//boşluk ve tre leri al yerine bişey koyma
            if (cardNumber.Length!=16) return false; //kart numarsı 16 karakter degilse
            foreach (var chr in cardNumber)//herdefasında gelen kartın içindeki degere bakıcak
            {
                if (!char.IsNumber(chr)) return false;  //rakam değilse
               
            }
            int oddTotal = 0;//tek
            int ovenTotal = 0;//çift
        ...
        private Payment PaymentProcess (OrderDto orderDto)
        {
            ...
            options.ApiKey = "sandbox-LAkgPZ4WQtWwAKrXrILtIsXWmP215Q0f";
            ...
            CreatePaymentRequest request = new CreatePaymentRequest
            {
                ...
                Price = Convert.ToUInt32(orderDto.CardDto.TotalPrice()).ToString(),//decimaldan inte çevirip sonra stringe çeviridk yaniküsüratı attık
                PaidPrice = Convert.ToUInt32(orderDto.CardDto.TotalPrice()).ToString(),
                ...
                Buyer = new Buyer
                {
                    ...
                    Email="wissen@gmial.com",
                    IdentityNumber = "7500880099547",
            ...
            foreach (var item in orderDto.CardDto.CardItems)
            {
                ...
                basketItem.Category1 = "Genel";//bu bize aslında kartdto içindeki productların kategroi bilgileirni doldurmmazı gerektiğini anlattı
                basketItem.ItemType = BasketItemType.PHYSICAL.ToString();
                basketItem.Price = Convert.ToUInt32(item.ItemPrice * item.Quantity).ToString();//küsüratı atmış olduk
                basketItems.Add(basketItem);//listeye ekleme yapıcak
            }
