    ...
    public class EfCoreProductRepository : EfCoreGenericRepository<Product>, IProductRepository
    {
        ...
        public async Task<List<Product>> GetSearchResultsAsync(string searchString,bool isApproved=true)
        {
            ...
            if (isApproved)
            {
                //onaylıysa gözükücek
                return  await result
                 .Where(p => p.IsApproved && (p.Name.ToLower().Contains(searchString) ||
                 p.Description.ToLower().Contains(searchString))).ToListAsync();//ismin küçük harfine bak dedik (onaylı olması lazım ve ardığımız ismin geçmesi lazım veya açıklama da içeriyorm)
            ...
            p.Description.ToLower().Contains(searchString)).ToListAsync();
