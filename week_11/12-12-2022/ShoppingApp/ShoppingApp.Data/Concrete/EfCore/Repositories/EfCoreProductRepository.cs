...
namespace ShoppingApp.Data.Concrete.EfCore.Repositories
{
    public class EfCoreProductRepository : EfCoreGenericRepository<Product>, IProductRepository
    {
        ...
        {
            //önc eproduct kaydı yapıcagız böylece elimizde productId olucak
            
             await ShopAppContext.Products.AddAsync(product);
             await ShopAppContext.SaveChangesAsync();
            product.ProductCategories = selectedCategoryIds//ilgilil product ın categoryi bilgisni doldrufuk
                .Select(catId => new ProductCategory
                {
                    ProductId = product.Id,
                    CategoryId = catId//seçilen kategori Id
                }).ToList();
            await ShopAppContext.SaveChangesAsync();//daha sonra tekrar kaydediyoruz
        }
        ...
        {
           var products=  ShopAppContext.Products.AsQueryable();//var olan productları getricek
            if (category!=null)
            {
                products =  products
        ...
        public async Task<List<Product>> GetProductsWithCategories()
        {
            ...
            return await ShopAppContext//shopApp den product sa eriş
                .Products
                .Include(p => p.ProductCategories)//productların içinde dön Include ile proudct bilgilerini dahil et(yani her p nin productCategoriesine geç)
                .ThenInclude(pc => pc.Category)//birinnci üürndeyken productCategroisdnen category bilgisni al demiş oluyotux
                .ToListAsync();
        }
